function res=PAResult(taskinfo)
    ...
    res = tlist(['PAResult','cleanDir', 'outFile','jobid','taskid', 'cleaned', 'logsPrinted','logs','waited','iserror','resultSet','dbrid','RaL'], taskinfo.cleanDir, taskinfo.outFile, taskinfo.jobid, taskinfo.taskid, jnewInstance(cz_ab,%f), jnewInstance(cz_ab,%f), jnewInstance(cz_sb), jnewInstance(cz_ab,%f), jnewInstance(cz_ab,%f),jnewInstance(cz_ab,%f), dbrid, jnewInstance(cz_ur));
    jremove(cz_ab);
    jremove(cz_ur);
    jremove(cz_sb);
endfunction
