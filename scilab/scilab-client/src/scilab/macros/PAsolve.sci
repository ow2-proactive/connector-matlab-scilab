...
function initSolveConfig(solve_config,opt)
    ...
    if ~isdir(worker_lib_dir) then
        error('PAsolve::cannot find directory  defined in option WorkerJarsDir : ' + worker_lib_dir);
    end

    fs=filesep();
    jarfiles = listfiles(worker_lib_dir+fs+'*.jar');

    for i=1:size(jarfiles,1)
        jinvoke(solve_config,'addWorkerJar',jarfiles(i));
