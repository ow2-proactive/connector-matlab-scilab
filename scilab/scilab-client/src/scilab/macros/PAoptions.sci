function opts = PAoptions(varargin)
    ...
    function y=ismember(a,l)
        y=(or(a==l));
    endfunction

    function y=versioncheck(x)
        if isnumeric(x)
            y=isempty(x);
        elseif ischar(x)
            y=~isempty(regexp(x, '/^[1-9][0-9]*(\.[0-9]+)*$/'));
        else
            y=%f;
        end
    endfunction

    function y=versionlistcheck(x)
        if isnumeric(x)
            y=isempty(x);
        elseif ischar(x)
            y=~isempty(regexp(x, '/^([1-9][0-9]*(\.[0-9]+)*[ ;,]+)*[1-9][0-9]*(\.[0-9]+)*$/'));
        else
            y=%f;
        end
    endfunction

    function y=jarlistcheck(x)
        if ischar(x)
            y=~isempty(regexp(x, '/^([\w\-\.]+\.jar[ ;,]+)*(([\w\-\.]+\.jar)|([*]))$/'));
        else
            y=%f;
        end
    endfunction

    function y=listcheck(x)
        if ischar(x)
            y=~isempty(regexp(x, '/^([^ ;,]+[ ;,]+)*[^ ;,]+$/'));
        else
            y=%f;
    ...
    function y=isstrictpositiveint(x)
        ss=evstr(x);
        if isnumeric(ss) & sum(length(ss))==1 & ss > 0 & floor(ss) == ss
            y=%t;
        else
            y=%f;
        end
    ...
    function y=variabletrans(x)
        y=strsubst(strsubst(strsubst(x, '$MATSCI$', PA_matsci_dir),'$TMP$',tmp_dir), '$HOME$', home_dir);
    endfunction

    function y=scripttrans(x)
        if ischar(x)
            y=strcat(['file:', strsubst(variabletrans(x), '\', '/')]);
        else
            y=x;
        end
    endfunction

    function y=conftrans(x)
        y=strsubst(variabletrans(x),'/',filesep());
    endfunction

    function y=ischarornull(x)
        if isnumeric(x)
            y=isempty(x);
        else
            y=ischar(x);
    ...
    function y=charornum(x)
        y=or(type(x)==[1,5,8,10]);
    endfunction

    function y=charornumtrans(x)
        if isnumeric(x)
            y=x;
        else
            y=evstr(x);
    ...
    function y=prioritycheck(x)
        if ischar(x)
            y=ismember(x,['Idle', 'Lowest', 'Low', 'Normal', 'High', 'Highest']);
        else
            y=%f;
        end
    endfunction

    function y=versionarchcheck(x)
        if ischar(x)
            y=ismember(x,['any', '32', '64']);
        else
            y=%f;
        end
    endfunction
    ...
    inputs(j).default = '$MATSCI$'+ fs +'lib' + fs + 'client';
    inputs(j).check = 'ischar';
    inputs(j).trans = 'conftrans';
    j=j+1;

    inputs(j).name = 'MiddlemanJarsDir';
    inputs(j).default = '$MATSCI$'+ fs +'lib' + fs + 'middleman';
    inputs(j).check = 'ischar';
    inputs(j).trans = 'conftrans';
    j=j+1;
    inputs(j).name = 'WorkerJarsDir';
    inputs(j).default =  '$MATSCI$'+ fs +'lib' + fs + 'worker';
    inputs(j).check = 'ischar';
    inputs(j).trans = 'conftrans';
    j=j+1;
    ...
    if isempty(proactive_pa_options) then 
        ...
        try
            ...
            mclose(fid);
            [str2,n2,line2,func2]=lasterror(%t);mprintf('!-- error %i %s at line %i of function %s',n2,str2,line2,func2);
