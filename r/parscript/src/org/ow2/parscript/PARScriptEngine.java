...
/**
 * R implementation of ScriptEngine using REngine (JRI or Rserve).
 * Sub-class of the RScriptEngine, adds support for types of objects filled into bindings by 
...
public class PARScriptEngine extends RScriptEngine {
	...
	public static PARScriptEngine create() {
		...
		try {
			paRengine.engine = REngine.engineForClass(cls, args, paRengine, /*runREPL*/ false);
		} catch (Exception e) {
	...
	public Object eval(String script, ScriptContext context) throws ScriptException {
		...
		try {
			...
			// environment it is considered as the task result instead of the result exp
		...
		if (!this.callbackedErrorMessages.isEmpty()) {
			String mess = StringUtils.join(this.callbackedErrorMessages, System.getProperty("line.separator"));			
	...
	private void assignProgress(Bindings bindings) {		
		...
		try {
			...
			super.engine.parseAndEval("set_progress = function(x) { .jcall(jTaskProgress, \"V\", \"set\", as.integer(x)) }");
		} catch (REXPMismatchException e) {
			e.printStackTrace();
		} catch (REngineException e) {
			e.printStackTrace();
	...
	public void RWriteConsole(REngine eng, String msg, int otype) {
		...
		if ( otype != 0 ){										
			...
			System.out.println("PARScriptEngine.RWriteConsole() --> " + otype);
