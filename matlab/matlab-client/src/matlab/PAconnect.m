...
if ~exist(middleman_lib_dir,'dir')
    error(['PAconnect::cannot find directory defined in option MiddlemanJarsDir : ' middleman_lib_dir]);
end

dir_content = dir([middleman_lib_dir filesep '*.jar']);
jarsjava = javaArray('java.lang.String', length(dir_content));
for i=1:length(dir_content)
    jarsjava(i) = java.lang.String([middleman_lib_dir filesep dir_content(i).name]);
end
