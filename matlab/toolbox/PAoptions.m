...
matsci_dir = char(javafile.getParentFile().getParent().toString());
...
inputs(j).default = ['$MATSCI$' filesep 'matlab' filesep 'script' filesep 'reserve_matlab.rb' ];
inputs(j).check = @ischar;
inputs(j).trans = @scripttrans;
j=j+1;
inputs(j).name = 'FindMatlabScript';
inputs(j).default = ['$MATSCI$' filesep 'matlab' filesep 'script' filesep 'file_matlab_finder.rb' ];
inputs(j).check = @ischar;
inputs(j).trans = @scripttrans;
j=j+1;
...
if ~exist('pa_options','var') == 1 || ~isstruct(pa_options)
    ...
    if exist(optionpath, 'file');
        ...
        optionpath = [matsci_dir filesep 'matlab' filesep 'config', filesep, 'toolbox', filesep, 'PAoptions.ini'];
        fid = fopen(optionpath, 'r');
    end
    try
        C = textscan(fid, '%s = %[^\n]', 'CommentStyle', '%');
